...
const RuleScoreBar = ({ student, rules, studentRuleScores, allStudents }) => {
  ...
  return (
    <div className="relative w-full h-6" title={`총점: ${totalScore}점 (양수: +${totalPositive}, 음수: -${totalNegative})`}>
      ...
      {/* 실제 점수 막대 (액체) - 위아래 패딩 */}
      <div className="absolute inset-x-0 inset-y-1 flex overflow-hidden rounded">
        {hasNegative ? (
          <>
            {/* 음수 영역 (0 지점 왼쪽) */}
            <div 
          ...
          return (
            <div key={student.id} className="relative">
              ...
              <div className="relative w-full h-10">
                ...
                {/* 실제 점수 막대 (액체) - 위아래 패딩 */}
                <div className="absolute inset-x-0 inset-y-1.5 flex overflow-hidden rounded">
                  {rangeValues.hasNegative ? (
                    <>
                      {/* 음수 영역 (0 지점 왼쪽) */}
                      <div 
                  ...
                  <tr key={student.id} className={`transition duration-150 ease-in-out ${index % 2 === 0 ? 'hover:bg-gray-50' : 'hover:bg-indigo-50'} ${index < 3 ? 'bg-yellow-50/50 font-bold' : ''}`}>
                    ...
                    <td className="px-1 sm:px-2 md:px-3 py-2 sm:py-4 whitespace-nowrap text-sm text-center">
                      <div className="w-12 sm:w-24 md:w-32 lg:w-40 mx-auto">
                        <RuleScoreBar student={student} rules={rules} studentRuleScores={studentRuleScores} allStudents={data.leaderboard} />
                      </div>
                    </td>
                  </tr>
                ))}
